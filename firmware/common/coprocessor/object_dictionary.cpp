...
// Optional dev cache-buster header, generated by build.sh (see the -f / --force-esp-reflash flag). When present with a
// nonzero ADSBEE_FORCE_REFLASH_BUILD_ID, a per-build value (the build timestamp) is mixed into the low 16 bits of
// kFirmwareVersion. build.sh writes the same value into the header for every processor in a single build run, so the
// RP2040 and coprocessor images agree after flashing; a subsequent rebuild produces a new value, so the RP2040 detects
// a version mismatch and reflashes the coprocessors even when the release-candidate number wasn't bumped. __has_include
// keeps this a no-op for builds that don't go through build.sh (e.g. IDE/CI direct CMake, host unit tests).
#if defined(__has_include)
#if __has_include("firmware_build_id.hh")
#include "firmware_build_id.hh"
#endif
#endif
#ifndef ADSBEE_FORCE_REFLASH_BUILD_ID
#define ADSBEE_FORCE_REFLASH_BUILD_ID 0
#endif

#if ADSBEE_FORCE_REFLASH_BUILD_ID
const uint32_t ObjectDictionary::kFirmwareVersion =
    (kFirmwareVersionMajor << 24) | (kFirmwareVersionMinor << 16) | (ADSBEE_FORCE_REFLASH_BUILD_ID & 0xFFFF);
#else
